...
class Client_menu_model extends CI_Model 
{
    ...
    public  function get_menu_by_title($title=0) 
    {
        $this->db->select('*');
        $this->db->from($this->table);
    ...
    public function build_menu($menu_level=1,$username='', $menu_reference=0, $client_site_id=0)
    {
        $root_menu = '';
        $num_child = 0;
        $menu_id   = 0;
        ...
        $sql = 'SELECT "apf"."MENU_ID", 
                        ...
                        "mnn"."REMARK", 
                        "mnn"."TITLE",
                        "acs"."CLIENT_SITE_NAME"
                  FROM "APP_CLIENT_MENU" mnn 
                  INNER JOIN "APP_CLIENT_ACCESS" apf ON "mnn"."MENU_ID" = "apf"."MENU_ID" 
                  INNER JOIN "APP_CLIENT_USER" usr ON "apf"."NAME" = "usr"."FUNCTION_ACCESS" 
                  INNER JOIN "APP_CLIENT_SITE" acs ON "usr"."CLIENT_SITE_ID" = "acs"."CLIENT_SITE_ID" 
                  WHERE "mnn"."MENU_LEVEL" = \''.$menu_level.'\' AND "mnn"."REFERENCE" = \''.$menu_reference.'\' AND 
                        "mnn"."SHOW" = \'1\'AND "usr"."USERNAME" = \''.$username.'\' AND "acs"."CLIENT_SITE_ID" = \''.$client_site_id.'\' AND   
        ...
        if($query->num_rows() > 0) {
            ...
            foreach($query->result() as $row_menu) {
                ...
                $sql_access = 'SELECT "afa"."READ_PRIV" , 
                                      ...
                                      "afa"."DELETE_PRIV" ,
                                      "afa"."ADD_PRIV"
                               FROM "APP_CLIENT_USER" usr 
                               INNER JOIN "APP_CLIENT_ACCESS" afa on "usr"."FUNCTION_ACCESS" = "afa"."NAME" 
                               INNER JOIN "APP_CLIENT_SITE" acs ON "usr"."CLIENT_SITE_ID" = "acs"."CLIENT_SITE_ID" 
                               WHERE "usr"."USERNAME" = \''.$username.'\' AND "afa"."MENU_ID" = \''.$menu_id.'\'  
                               AND "acs"."CLIENT_SITE_ID" = \''.$client_site_id.'\'';
                ...
                if($query_access->num_rows() > 0) {
                    ...
                    if($read == 1) {
                        ...
                        $sql_child   = 'SELECT * FROM "APP_CLIENT_MENU" where "REFERENCE" = \''.$menu_id.'\' AND "SHOW" = \'1\' AND coalesce("IS_DELETE",0) != \'1\'';
                        ...
                        $sql_check_menu   = 'SELECT COUNT(*) FROM "APP_CLIENT_MENU" WHERE "MENU_LEVEL" = \''.$check_level.'\' AND "REFERENCE" = \''.$menu_id.'\'';
                        $query_check_menu = $this->db->query($sql_check_menu);
                        
                        if($query_check_menu->num_rows() > 0) {
                            $root_menu .= $this->build_menu($check_level, $username,$menu_id, $client_site_id);
                        }
